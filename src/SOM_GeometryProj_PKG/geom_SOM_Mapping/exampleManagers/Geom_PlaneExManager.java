...
public class Geom_PlaneExManager extends SOM_GeomExampleManager {
	...
	protected void buildMTLoader(String[] loadSrcFNamePrefixAra, int numPartitions) {
		...
		for (int i=0; i<numPartitions;++i) {	preProcLoaders.add(new Geom_PlaneCSVDataLoader((SOM_GeomMapManager) mapMgr, i, loadSrcFNamePrefixAra[0]+"_"+i+".csv",  exampleName+ " Data file " + i +" of " +numPartitions + " loaded",  exampleName+ " Data File " + i +" of " +numPartitions +" Failed to load", curDataType, exampleMap));}	
	...
	protected SOM_Example buildSingleExample(String _oid, String _str) {
		return new Geom_PlaneSOMExample(((Geom_PlaneMapMgr)mapMgr), curDataType, _oid, _str);
	}
