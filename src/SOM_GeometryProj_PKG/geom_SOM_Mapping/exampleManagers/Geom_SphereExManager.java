...
public class Geom_SphereExManager extends SOM_GeomExampleManager {
	...
	protected void buildMTLoader(String[] loadSrcFNamePrefixAra, int numPartitions) {
		...
		for (int i=0; i<numPartitions;++i) {	preProcLoaders.add(new Geom_SphereCSVDataLoader((SOM_GeomMapManager) mapMgr, i, loadSrcFNamePrefixAra[0]+"_"+i+".csv",  exampleName+ " Data file " + i +" of " +numPartitions + " loaded",  exampleName+ " Data File " + i +" of " +numPartitions +" Failed to load", curDataType, exampleMap));}	
