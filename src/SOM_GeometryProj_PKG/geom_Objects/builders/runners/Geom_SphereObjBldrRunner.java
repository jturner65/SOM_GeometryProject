...
import SOM_GeometryProj_PKG.geom_Objects.builders.callables.Geom_SphereObjBuilder;
...
public class Geom_SphereObjBldrRunner extends SOM_GeomObjBldrRunner {
	...
	public Geom_SphereObjBldrRunner(SOM_MapManager _mapMgr, ExecutorService _th_exec, boolean _forceST, int _numObjs, int _taskType) {
		super(_mapMgr, _th_exec, "Spheres", _forceST, _numObjs, _taskType);
	}
	...
	protected void execPerPartition(List<Callable<Boolean>> ExMappers, int dataSt, int dataEnd, int pIdx,int ttlParts) {
		ExMappers.add(new Geom_SphereObjBuilder(msgObj, dataSt, dataEnd, pIdx, taskType, dataTypeName));
	}
	...
	protected void runMe_Indiv_ST() {
		...
		Geom_SphereObjBuilder rnr = new Geom_SphereObjBuilder(msgObj, 0, numObjs, 0, taskType, dataTypeName);
		try {			rnr.call();		} 
		catch (Exception e) {
			e.printStackTrace();
		}
